...
namespace TicTacToeGame
{
    ...
    public class GameHub : Hub
    {
        ...
        public Task OnOpponentDisconnected(string connectionId, string playerName)
        {
            ...
            return Clients.Client(connectionId).SendAsync(Constants.OpponentDisconnected, playerName);
        }
        ...
        public void MakeAMove(int position)
        {
            ...
            Clients.Client(game.Player1.ConnectionId).SendAsync(Constants.MoveMade, new MoveInformation { OpponentName = player.Name, ImagePosition = position, Image = player.Image });
            Clients.Client(game.Player2.ConnectionId).SendAsync(Constants.MoveMade, new MoveInformation { OpponentName = player.Name, ImagePosition = position, Image = player.Image });
            ...
            {
                Remove<Game>(games, game);
                Clients.Client(game.Player1.ConnectionId).SendAsync(Constants.GameOver, "Its a tame draw!!!");
                Clients.Client(game.Player2.ConnectionId).SendAsync(Constants.GameOver, "Its a tame draw!!!");
                player.IsPlaying = false;
                player.Opponent.IsPlaying = false;
                this.Clients.Client(player.ConnectionId).SendAsync(Constants.RegistrationComplete);
                this.Clients.Client(player.Opponent.ConnectionId).SendAsync(Constants.RegistrationComplete);
        ...
        public void FindOpponent()
        {
            ...
            Clients.Client(Context.ConnectionId).SendAsync(Constants.OpponentFound, opponent.Name, opponent.Image);
            Clients.Client(opponent.ConnectionId).SendAsync(Constants.OpponentFound, player.Name, player.Image);
            ...
            if (toss.Next(0, 1) == 0)
            {
                ...
                Clients.Client(player.ConnectionId).SendAsync(Constants.WaitingForMove, opponent.Name);
                //// Tell opponent player to wait for the move
                Clients.Client(opponent.ConnectionId).SendAsync(Constants.WaitingForOpponent, player.Name);
            }
            else
            {
                ...
                Clients.Client(opponent.ConnectionId).SendAsync(Constants.WaitingForMove, player.Name);
                //// Tell player to wait for the opponent's move
                Clients.Client(player.ConnectionId).SendAsync(Constants.WaitingForOpponent, opponent.Name);
            }
